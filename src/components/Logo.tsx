...
const Logo = ({ size = 'md', lang }: { size?: 'sm' | 'md' | 'lg', lang: string }) => {
  const iconSize = size === 'sm' ? 20 : size === 'md' ? 24 : 32;
  const textSize = size === 'sm' ? 'text-lg' : size === 'md' ? 'text-xl' : 'text-2xl';

  return (
    <Link href={`/${lang}`} className="flex items-center gap-2 text-primary hover:text-primary/90 transition-colors">
